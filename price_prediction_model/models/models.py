...
def polynomialRegression(X, Y, X_train, Y_train, X_test, Y_test, X_validate, Y_validate):
    ...
    print("Polynomial Regression with degree = {} and rmse = {}".format(selected_degree, min_poly_validate_rmse))
    
    # Apply Ridge Regression:
    alphas = [0.00001, 0.00003, 0.00005, 0.00008, 0.0001, 0.0003, 0.0005, 0.0008, 0.001, 0.005, 0.01, 0.02, 0.04, 0.06, 0.1, 1.0, 3.0, 10.0, 50.0, 100.0]

    selected_alpha = alphas[0]
    selected_regularized_model = ridgeRegressionModel(selected_X_train_poly, Y_train, selected_alpha)
    # model = lassoRegressionModel(selected_X_train_poly, Y_train, alphas[0])

    X_validate_poly = polynomialTransform(X_validate, selected_degree)
    min_regularized_validate_rmse = calcRMSE(selected_regularized_model, X_validate_poly, Y_validate)
    ...
    for alpha in alphas[1:]:
        ...
        X_validate_poly = polynomialTransform(X_validate, selected_degree)
        validate_rmse = calcRMSE(model, X_validate_poly, Y_validate)

        if validate_rmse < min_regularized_validate_rmse:
            min_regularized_validate_rmse = validate_rmse
            selected_alpha = alpha
            selected_regularized_model = model

    print("Regularized Regression with alpha = {} and rmse = {}".format(selected_alpha, min_regularized_validate_rmse))

    # Choose between Ridge Regression and Polynomial Regression using cross validation score:
    poly_model_cv_score = calcCV(selected_poly_model, X, Y)
    print("Polynomial Regression cross validation score: ", poly_model_cv_score)

    regularized_model_cv_score = calcCV(selected_regularized_model, X, Y)
    print("Regularized Regression cross validation score: ", regularized_model_cv_score)

    selected_model = selected_poly_model if poly_model_cv_score > regularized_model_cv_score else selected_regularized_model
    selected_validate_rmse = min_poly_validate_rmse if selected_model == selected_poly_model else min_regularized_validate_rmse
